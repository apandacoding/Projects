...
def next_perfect_square(score2):
    if perfect_square(score2) == True:
        s1 = int((score2**0.5)+1)
        return s1**2
    elif(perfect_square(score2) == False):
        return score2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    # for any given combination of scores shared between player1 and player2 will the strategy always pick the same amount of dice to roll?

    player_score = 0
    while(player_score < goal):
        opponent_score = 0
        while(opponent_score < goal):
            if(strategy(player_score,opponent_score) != strategy(0,0)):
                return False
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    y = 1
    while x <= 10:
        average = make_averaged(roll_dice, total_samples)(x, dice)
        if (average > max):
            max = average
            y = x
        x += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score = score + tail_points(opponent_score)
