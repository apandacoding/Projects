...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            #u can add a second ant to place only when exactly one of the Ant instances is a container and the container ant does not already contain another ant
            # If two Ants occupy the same Place, what is stored in that place's ant inst nce attribt
            # BEGIN Problem 8b
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # in problem 3 we need to find a way to loop through the places and return a bee if there is a bee or if there's not return None
        # starting at currents ants place and as we change this current place and find a bee in current place return 
        # return random_bee(place)
        # END Problem 3 and 4
        current = self.place # represents the ants strarting place and place object
        i = 0
        # if current.entrance is None that means entrance 
        while(current.entrance != None and type(current) != type(Hive)):
            if(len(current.bees) > 0  and i >= self.lower_bound and i <= self.upper_bound):
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        current = self.place 
        if(self.turns_to_chew == 0 and len(current.bees) > 0 ):
        ...
        while current:
            if(current.ant != None):
                if(current.ant.is_container and current.ant.ant_contained != None):
                    if not current.ant.ant_contained.is_doubled:
                        current.ant.ant_contained.double()
                        current.ant.ant_contained.is_doubled = True
...
class SlowThrower(ThrowerAnt):
    ...
    implemented = True   # Change to True to view in the GUI
    # END Problem EC
    def ant_work(self, gamestate):
        if self.turns == 0:
            self.action = Bee.action.__get__(self, self.__class__)
    ...
    def throw_at(self, target):
        ...
        if target != None:
            target.turns = 5
            target.action = SlowThrower.ant_work.__get__(target, target.__class__)
        # END Problem EC
